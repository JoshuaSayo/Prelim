...
if (isset($_POST['saveImg'])) {
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        $updateImg = mysqli_query($db, "UPDATE tbl_board SET img = '$image', updated_by = '$updated_by', last_updated = CURRENT_TIMESTAMP WHERE board_id = '$board_id'") or die(mysqli_error($db));
...
if (isset($_POST['save'])) {
    ...
    if ($check == 0) {
        $q = $db->query("SELECT * FROM tbl_board WHERE username = '$username'") or die($db->error);
        $check2 = mysqli_num_rows($q);
        while ($row = mysqli_fetch_array($q)) {
            $getID = $row['board_id'];
        }
        if ($getID == $board_id || $check2 < 1) {
            $updateInfo = mysqli_query($db, " UPDATE tbl_board SET board_lastname='$lname',board_firstname='$fname', board_middlename='$mname', email='$email', username='$username', updated_by = '$updated_by', last_updated = CURRENT_TIMESTAMP WHERE board_id = '$board_id'") or die(mysqli_error($db));
...
if (isset($_POST['savePass'])) {
    ...
    if ($_SESSION['role'] == "Board") {
        ...
        $checkPass = mysqli_query($db, "SELECT * FROM tbl_board WHERE board_id = '$board_id'");
        while ($row = mysqli_fetch_array($checkPass)) {
            ...
            if ($checkHashPass == false) {
                ...
                if ($password == $confirmPass) {
                    ...
                    $updatePass = mysqli_query($db, " UPDATE tbl_board SET password='$hashedPwd', updated_by = '$updated_by', last_updated = CURRENT_TIMESTAMP WHERE board_id = '$board_id'") or die(mysqli_error($db));
