...
if (isset($_POST['signin']) || isset($_SESSION['confirm'])) {
    ...
    $board    = mysqli_query($db, "SELECT * FROM tbl_board WHERE username = '$username'");
    $numrow9 = mysqli_num_rows($board);
    ...
    if ($numrow > 0) {
        ...
        while ($row = mysqli_fetch_array($board)) {
            ...
            if ($hashedPwdCheck == false) {
                ...
                $_SESSION['role'] = "Board";
                $_SESSION['userid'] = $row['board_id'];
                $_SESSION['name'] = $row['board_lastname'] . ", " . $row['board_firstname'];
            }
            header("location: ../../dashboard/index.php");
        }
    } else {
