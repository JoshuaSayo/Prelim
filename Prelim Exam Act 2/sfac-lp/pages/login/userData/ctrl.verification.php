...
if (isset($_POST['submit']) && !empty($_SESSION['username'])) {
    ...
    if (!empty($code)) {
        ...
        $board    = $db->query("SELECT * FROM tbl_board WHERE username = '$username'");
        $numrow1 = $board->num_rows;
    ...
    if ($numrow > 0) {
        ...
        $query1 = $db->query("UPDATE tbl_board SET activation_code = '$randNum' WHERE username = '$username'");
    } else {
        $_SESSION['invalid_email'] = true;
        header("location: ../reset_password.php");
        exit();
